...
(function() {
  function scheduleGlitch(el, minMs, maxMs) {
    setTimeout(function fire() {
      ...
      setTimeout(() => { el.classList.remove('is-glitching'); scheduleGlitch(el, minMs, maxMs); }, 150 + Math.random() * 100);
    }, minMs + Math.random() * (maxMs - minMs));
  }
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.hero-title.glitch, .page-title.glitch').forEach(el => scheduleGlitch(el, 2000, 6000));
    const trophy = document.querySelector('.trophy-wrap.glitch');
    if (trophy) scheduleGlitch(trophy, 2500, 7000);
  });
})();
